...
async def semantic_search(request: SearchRequest):
    ...
    try:
        ...
        results = vector_loader.search(
            request.query, k=request.k or 5, use_adaptive_threshold=True
        )

        return SearchResponse(
            results=results, query=request.query, timestamp=datetime.now().isoformat()
        )
    except Exception as e:
...
async def conversational_rag(request: ChatRequest):
    ...
    try:
            ...
            for msg in request.history:
                if msg.role == "user":
                    add_to_conversation_history(
                        request.session_id, HumanMessage(content=msg.content)
                    )
                elif msg.role == "assistant":
                    add_to_conversation_history(
                        request.session_id, AIMessage(content=msg.content)
                    )
        ...
        if not retrieved_docs:
            ...
            add_to_conversation_history(
                request.session_id, HumanMessage(content=request.message)
            )
            add_to_conversation_history(
                request.session_id, AIMessage(content=refusal_response)
            )
