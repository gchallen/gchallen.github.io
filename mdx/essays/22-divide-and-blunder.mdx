...
As we'll see, divide and blunder both fails to leverage advantages inherent to large courses, while creating additional problems that negatively affect students and the curriculum.
...
Let's use the example of a 1000-student course, and work from the reasonable end of the divide and blunder spectrum to the unreasonable end.
...
Note that this can also be helpful in making the course more available to students by reducing scheduling conflicts.
...
In the face of these forces driving divergence, there's rarely anyone championing consistency.
...
You're more likely to end up with inexperienced graduate students, poorly-paid adjuncts, or faculty busy with too many other things—and nobody who will stick around for long.
...
Rather than the alternative, where excellent instructors create materials that benefit every student.
...
These knowledge consistency problems have to be addressed somewhere.
Frequently, downstream courses now have to reteach large amounts of material that students have already paid to learn.
...
A well-run follow-on course will calibrate on the least-prepared "successful" incoming students, meaning that a single poorly-taught independent section can ruin the entire batch.

Consistency problems can also confuse and demoralize students.
...
Finally, don't think that I'm arguing that addressing consistency problems will fix all knowledge gaps that may affect students in later courses.
...
But it's just much easier for a later course to adjust when all students have been taught in the same way, and are share similar knowledge gaps.
...
But even shared assessments may not be sufficient to overcome another significant confounder: registration.
...
Which I guess is understandable, given that nobody can answer that question.
...
Students may have earlier or later access to certain courses for all kinds of more or less comprehensible reasons: how close they are to graduation, whether they are majoring in the subject, status in various honors programs, and so on.
I could and may write a separate essay complaining about this registration ordering system, which isn’t designed to accomplish a coherent objective—and so, unsurprisingly, doesn't achieve one.
But for our purposes it suffices to say that the students who enroll in separate sections of the same course are nowhere close to a representative sample.
...
But this just isn’t being done today, at least not unless someone wants to write a research paper.
And even if you could determine which section was more effective, pinpointing the reason why would be equally challenging, particularly given that independent sections usually diverge in multiple ways.

What happens today is simpler and sadder: independent sections freely create significant consistency problems.
Either the next course in the sequence must correct these disparities, or the students who are left behind give up.
...
In the final installment of this series, I'll provide ideas about how to structure and staff large courses so that they not only _should_ be great, but _can_ be.
