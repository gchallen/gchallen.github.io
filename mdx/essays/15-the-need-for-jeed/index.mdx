...
The security manager allows you to enable runtime checks that are triggered by a variety of different program behaviors.
If the program tries to read from a file, for example, the security manager allows you to install code that examines that read request and determines whether to allow it to proceed. 
...
The Java virtual machine can install a security manager before the virtual machine begins executing a program, and so provides a way to control access to your system when running a Java binary.
But it also allows a security manager to be installed by a program itself as it runs, which allows us to selectively sandbox the execution of just a portion of the program—specifically, the untrusted student code.
...
Ben wrote code that examines all `try-catch` blocks in compiled bytecode and, if it finds ones that would catch `ThreadDeath`, rewrites them to prevent that.
Meaning that any code that we load into the sandbox can always be stopped.

More technical readers will note at this point that what we have actually performed is what is called an unclean shutdown.
...
Ben has also assured me that we could establish a similar level of sandboxing using bytecode rewriting.
Given how good he's gotten at it, I don’t doubt it.
